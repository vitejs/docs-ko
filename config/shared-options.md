...
캐시 파일을 저장할 디렉터리입니다. 이 디렉터리의 파일은 사전 번들링된 디펜던시 또는 Vite가 생성한 기타 캐시 파일이며, 성능을 개선할 수 있습니다. 캐시 파일을 다시 생성하려면 `--force` 플래그를 사용하거나 디렉터리를 직접 삭제할 수 있습니다. 값은 절대 파일 시스템 경로이거나 프로젝트 루트 기준 상대 경로일 수 있습니다. `package.json`이 감지되지 않으면 기본값은 `.vite`입니다.
...
`import` 또는 `require` 문에서 값을 대체하는 데 사용할 별칭을 정의합니다. 이는 [`@rollup/plugin-alias`](https://github.com/rollup/plugins/tree/master/packages/alias)와 유사하게 동작합니다.

항목의 순서는 중요합니다. 먼저 정의된 규칙이 먼저 적용됩니다.
...
### 객체 형식 (`Record<string, string>`) {#object-format}

객체 형식은 키로 별칭을 지정하고, 해당 값을 실제 임포트 값으로 지정할 수 있습니다. 예시는 다음과 같습니다:
...
### 배열 형식 (`Array<{ find: string | RegExp, replacement: string }>`) {#array-format}

배열 형식은 별칭을 객체로 지정할 수 있게 하며, 복잡한 키/값 쌍에 유용할 수 있습니다.
...
`find`가 정규 표현식인 경우, `replacement`는 `$1` 같은 [치환 패턴](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/replace#specifying_a_string_as_the_replacement)을 사용할 수 있습니다. 예를 들어 확장자를 다른 값으로 제거하려면 다음과 같은 패턴을 사용할 수 있습니다:
...
또한 스타일 임포트를 해석할 때는 `@import 'my-library'`와 같이 `style` 조건이 적용됩니다. 일부 CSS 전처리기에서는 Sass의 `sass`, Less의 `less`처럼 해당 조건도 적용됩니다.
...
tsconfig paths 해석 기능을 활성화합니다. `tsconfig.json`의 `paths` 옵션이 임포트 해석에 사용됩니다. 자세한 내용은 [기능](/guide/features.md#paths)을 참고하세요.
...
::: tip 파일 임포트
같은 코드가 서로 다른 디렉터리의 파일 앞에 추가되므로 상대 경로는 올바르게 해석되지 않습니다. 대신 절대 경로나 [별칭](#resolve-alias)을 사용하세요.
:::
...
- **실험적 기능:** [피드백을 남겨주세요](https://github.com/vitejs/devtools/discussions)
