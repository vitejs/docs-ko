...
const pastNotableDependencies: PastDependency[] = [
  {
    name: 'esbuild',
    description:
      'JavaScript/TypeScript 번들러 및 미니파이어(현재는 Rolldown, Oxc, LightningCSS 사용)',
    repository: 'https://github.com/evanw/esbuild',
  },
  {
    name: 'rollup',
    description: 'ES module 번들러(현재는 Rolldown 사용)',
    repository: 'https://github.com/rollup/rollup',
  },
  {
    name: 'http-proxy',
    description: 'HTTP 프록시(현재는 http-proxy-3 사용)',
    repository: 'https://github.com/http-party/node-http-proxy',
  },
  {
    name: 'acorn',
    description: 'JavaScript 파서',
    repository: 'https://github.com/acornjs/acorn',
  },
  {
    name: 'fast-glob',
    description: '빠른 glob 매칭(현재는 tinyglobby/fdir 사용)',
    repository: 'https://github.com/mrmlnc/fast-glob',
  },
  {
    name: 'debug',
    description: '디버그 로깅(현재는 obug 사용)',
    repository: 'https://github.com/debug-js/debug',
  },
]

const docsRootDir = path.resolve(import.meta.dirname, '..')
const upstreamVitePackageDir = path.resolve(
  import.meta.dirname,
  '../../packages/vite',
)
const vitePackageDir = fs.existsSync(upstreamVitePackageDir)
  ? upstreamVitePackageDir
  : path.join(docsRootDir, 'node_modules/vite')
...
function loadData(): AcknowledgementsData {
  ...
  const rootNodeModulesDir = fs.existsSync(
    path.resolve(import.meta.dirname, '../../node_modules'),
  )
    ? path.resolve(import.meta.dirname, '../../node_modules')
    : path.join(docsRootDir, 'node_modules')
