...
Module Runner API를 사용할 때는 `server.ssrFixStacktrace`와 `server.ssrRewriteStacktrace`를 호출할 필요가 없습니다. `sourcemapInterceptor`가 `false`로 설정되어 있지 않다면 스택 트레이스가 업데이트됩니다.
