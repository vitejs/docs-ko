...
## NPM을 이용한 디펜던시 `import` 그리고 Pre-Bundling
...
Vite은 기본적으로 Node.js API 기반의 타입 시스템을 차용하고 있습니다. 따라서 Client-side의 환경을 위해 Shim을 구성하고자 한다면, `tsconfig` 내 `compilerOptions.types` 옵션에 `vite/client`를 명시해 줘야만 합니다.
...
웹 워커의 경우 `?worker` 문자열을 붙여 Import 할 수 있습니다. 모듈의 `export default`로는 워커의 생성자가 들어가게 됩니다.
...
Vite은 네이티브 ESM을 이용한 Dynamic Import를 기반으로 동작합니다. 따라서 배포 시 이를 지원하지 않는 브라우저 또한 Dynamic Import를 사용할 수 있게끔 폴리필(Polyfill)이 필요하며, Vite은 이를 위해 [Dynamic import polyfill](https://github.com/GoogleChromeLabs/dynamic-import-polyfill)을 이용하고 있습니다.

만약 타깃으로 하고 있는 브라우저가 Dynamic Import를 모두 지원하고 있는 경우라면, [`build.polyfillDynamicImport`](/config/#build-polyfilldynamicimport) 옵션을 이용해 비활성화가 가능합니다.
