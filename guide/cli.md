...
| `--configLoader <loader>` | Rolldown으로 설정을 번들링하려면 `bundle`을, 즉석에서 처리하려면 `runner`(실험적 기능)를, 네이티브 런타임으로 로드하려면 `native`(실험적 기능)를 사용 (기본값: `bundle`) |
...
| Options                        |                                                                                                                          |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------ |
| `--target <target>`            | 트랜스파일 대상 (기본값: `"baseline-widely-available"`) (`string`)                                                       |
...
| `--minify [minifier]`          | 축소화를 활성화/비활성화하거나 사용할 minifier를 지정 (기본값: `"oxc"`) (`boolean \| "oxc" \| "terser" \| "esbuild"`) |
| `--manifest [name]`            | 빌드 매니페스트 JSON 내보내기 (`boolean \| string`)                                                                      |
| `--ssrManifest [name]`         | SSR 매니페스트 JSON 내보내기 (`boolean \| string`)                                                                       |
| `--emptyOutDir`                | outDir가 루트 밖에 있을 때 강제로 outDir 디렉터리를 비우기 (`boolean`)                                                   |
| `-w, --watch`                  | 디스크의 모듈이 변경되면 다시 빌드 (`boolean`)                                                                           |
| `-c, --config <file>`          | 설정 파일 지정 (`string`)                                                                                                |
| `--base <path>`                | `base` 옵션 위치 지정 (기본값: `/`) (`string`)                                                                           |
| `-l, --logLevel <level>`       | Info \| warn \| error \| silent (`string`)                                                                               |
| `--clearScreen`                | 로깅 시 화면을 지우는 것을 허용/비허용 (`boolean`)                                                                       |
| `--configLoader <loader>`      | Rolldown으로 설정을 번들링하려면 `bundle`을, 즉석에서 처리하려면 `runner`(실험적 기능)를 사용 (기본값: `bundle`)         |
| `--profile`                    | 빌트인 Node.js 인스펙터 실행 ([성능 병목현상](/guide/troubleshooting#performance-bottlenecks) 참고)                      |
| `-d, --debug [feat]`           | 디버그 로그 표시 (`string \| boolean`)                                                                                   |
| `-f, --filter <filter>`        | 디버그 로그 필터 (`string`)                                                                                              |
| `-m, --mode <mode>`            | env 모드 설정 (`string`)                                                                                                 |
...
이 명령어는 빌드 디렉터리(기본값: `dist`)에서 서버를 시작합니다. 빌드 디렉터리가 최신 상태인지 확인하려면 먼저 `vite build`를 실행하세요. 프로젝트에 설정된 [`appType`](/config/shared-options#apptype)에 따라 특정 미들웨어를 사용합니다.
